...
public class GlobalFog : PostEffectsBase
{
	...
	public virtual void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		Vector3 vector3 = camera.transform.right * CAMERA_NEAR * Mathf.Tan(num * ((float)Math.PI / 180f)) * CAMERA_ASPECT_RATIO;
		Vector3 vector4 = camera.transform.up * CAMERA_NEAR * Mathf.Tan(num * ((float)Math.PI / 180f));
		Vector3 vector5 = camera.transform.forward * CAMERA_NEAR - vector3 + vector4;
		float num2 = vector5.magnitude * CAMERA_FAR / CAMERA_NEAR;
		vector5.Normalize();
		vector5 *= num2;
		Vector3 vector6 = camera.transform.forward * CAMERA_NEAR + vector3 + vector4;
		vector6.Normalize();
		vector6 *= num2;
		Vector3 vector7 = camera.transform.forward * CAMERA_NEAR + vector3 - vector4;
		vector7.Normalize();
		vector7 *= num2;
		Vector3 vector8 = camera.transform.forward * CAMERA_NEAR - vector3 - vector4;
		vector8.Normalize();
		vector8 *= num2;
